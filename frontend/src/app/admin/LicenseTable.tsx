        ...
        <div className="aether-glass p-6 h-full flex flex-col">
          ...
          {licenses.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center py-12 border-2 border-dashed border-white/5 rounded-[16px]">
              ...
              <p className="text-[10px] uppercase tracking-widest text-[#A1A1AA]">{t.admin.noLicenses}</p>
            </div>
          ) : (
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-sm text-left">
                <thead className="text-[9px] uppercase tracking-widest text-[#A1A1AA] border-b border-white/10">
                  <tr>
                    <th className="py-3 px-4 font-medium">{t.admin.colKey}</th>
                    <th className="py-3 px-4 font-medium">{t.admin.colTier}</th>
                    <th className="py-3 px-4 font-medium">{t.admin.colDuration}</th>
                    <th className="py-3 px-4 font-medium">{t.admin.colStatus}</th>
                    <th className="py-3 px-4 font-medium">{t.admin.colUser}</th>
                    <th className="py-3 px-4 font-medium">{t.admin.colDates}</th>
                    <th className="py-3 px-4 font-medium text-right">{t.admin.colActions}</th>
                  </tr>
                </thead>
                  ...
                  {licenses.map((lic, i) => (
                    <motion.tr
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={`text-[9px] uppercase tracking-widest ${lic.is_used ? 'text-[#A1A1AA]' : 'text-[#6366F1]'}`}>
                            {lic.is_used ? t.admin.statusUsed : t.admin.statusReady}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4 text-[10px] text-[#A1A1AA]">
                        <div>{t.admin.created} {new Date(lic.created_at).toLocaleDateString("vi-VN")}</div>
                        {lic.used_at && <div className="mt-0.5 text-[#D4D4D8]">{t.admin.used} {new Date(lic.used_at).toLocaleDateString("vi-VN")}</div>}
                      </td>
