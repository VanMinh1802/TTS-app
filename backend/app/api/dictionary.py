...
from app.schemas.dictionary import (
    DictionaryCreate, DictionaryExport, DictionaryEntry,
    DictionaryImportRequest, DictionaryListResponse, DictionaryUpdate,
)
from app.core.exceptions import ConflictError, NotFoundError
from app.core.di import get_dictionary_service
...
async def list_dictionary(
    ...
    entries, total = service.list_entries(current_user.id, offset=offset, limit=page_size)
    return DictionaryListResponse(entries=entries, total=total, page=page, page_size=page_size)
...
async def update_entry(
    entry_id: str, update: DictionaryUpdate,
    current_user: User = Depends(get_current_user),
    service: DictionaryService = Depends(get_dictionary_service),
):
    try:
